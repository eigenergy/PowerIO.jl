...
`to_arrow(...; copy=true)` returns Julia owned vectors. `to_arrow(...; copy=false)` returns `ArrowColumn` views rooted by a shared `ArrowBuffers` owner. A column outlives its table until the table is closed.
...
Do not read an `ArrowColumn` after closing its parent `ArrowTable`. The Arrow hardening follow up changes this to a Julia error; until then, collect columns before closing when the owned vector must outlive the table.

Do not bypass the public API with `getfield(col, :data)` on an `ArrowColumn`. That field is an internal `unsafe_wrap` vector. Use normal indexing or `collect(col)`.
...
Do not close an `ArrowTable` concurrently with reading one of its columns from another Julia thread and then depend on either operation winning. Close is a mutating operation.
