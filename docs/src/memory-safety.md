...
- The internal DC data handle owns a `PioDcData *` behind
  `pio_dc_data_retain`/`pio_dc_data_release`.
...
P0, resolved: live handles crossed library overrides. `set_library!` (`src/capi.jl`) changes the global library path. Previously, a network parsed from one library still called `to_dense`, `to_arrow`, `emit`, module conversion, or distribution access through a different library selected by `set_library!`. Finalizers already captured the right free function, but normal reads still used the current library. Handles now store the allocating library, symbol lookup is cached per library, and every call that borrows a handle uses the handle library.
...
Parse and module API: `pio_parse_file`, `pio_parse_text`, `pio_module_emit_string`, `pio_module_emit_file`, `pio_module_kind`, `pio_module_diagnostics`, `pio_module_inspect_json`, `pio_module_list_states_json`, `pio_module_export_time_point`, `pio_module_export_scenario`, `pio_module_to_balanced_report_json`, `pio_module_to_balanced`, `pio_module_balanced_network`, `pio_module_multiconductor_network`, and the `pio_module_retain`/`pio_module_release` lifecycle. The released `pio_parse_str`, `pio_parse_bytes`, `pio_module_read_json`, `pio_module_write_json`, `pio_module_write_*`, `pio_module_state_inventory_json`, `pio_module_export_state`, `pio_module_lowering_readiness_json`, and `pio_module_lower_to_balanced` spellings remain ABI 6 compatibility or low level acquisition functions. Failures cross as `PioError` handles (`pio_error_code`, `pio_error_message`, `pio_error_release`), and findings as `PioDiagnostics` lists with per field accessors.

Balanced network API: `pio_balanced_network_from_json`, `pio_balanced_network_to_json`, `pio_balanced_network_to_normalized`, `pio_balanced_network_to_module`, `pio_balanced_network_to_geo_layer_json`, `pio_balanced_network_apply_geo_layer`, the count and table extractors (`pio_balanced_network_n_buses`, `..._n_branches`, `..._n_gens`, `..._base_mva`, `..._ref_bus_index`, `..._ref_bus_indices`, `..._n_islands`, `..._is_radial`, `..._bus_ids`, `..._branches`, `..._gens`, `..._bus_demand`, `..._bus_shunt`), and the `pio_balanced_network_retain`/`pio_balanced_network_release` lifecycle. The released `pio_balanced_network_normalize`, `pio_module_of_balanced_network`, `pio_balanced_network_geo_extract`, `pio_balanced_network_geo_apply`, `pio_convert_file`, and `pio_convert_str` functions remain ABI 6 compatibility paths.
...
Distribution API: `pio_multiconductor_network_summary_json`, `pio_multiconductor_network_to_json`, `pio_multiconductor_network_to_graph_json`, `pio_multiconductor_network_to_module`, `pio_multiconductor_network_to_geo_layer_json`, `pio_multiconductor_network_apply_geo_layer`, and the `pio_multiconductor_network_retain`/`pio_multiconductor_network_release` lifecycle; parsing and emission go through the module family above. The released `pio_multiconductor_network_graph_json`, `pio_module_of_multiconductor_network`, `pio_multiconductor_network_geo_extract`, and `pio_multiconductor_network_geo_apply` symbols remain ABI 6 compatibility aliases.

Private DC calculation support: `pio_dc_data_build`, `pio_dc_data_n_branches`, `pio_dc_data_branch_ids`, `pio_dc_data_calc_branch_flow`, the array accessors (`pio_dc_data_susceptance`, `..._from_indices`, `..._to_indices`, `..._bus_ids`, `..._shift`, `..._omitted_ids`, `..._omitted_reasons`), and the `pio_dc_data_retain`/`pio_dc_data_release` lifecycle. The Julia binding keeps this handle behind the direct `calc_*` methods. The row based `pio_dc_data_n_rows` and `pio_dc_data_row_ids` accessors and both `pio_dc_data_fill_branch_flow*` functions remain ABI 6 compatibility spellings.
