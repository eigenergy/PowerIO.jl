...
- `to_powerdata` allocates about a quarter less on the normalized path. Naming the element a field belongs to costs a string per field read — 25,801 of them on a 2869-bus case, ~7.2 MB — and every one of them is discarded on a case with nothing wrong with it. The element travels as data now and is rendered only inside the `throw`, so the messages are unchanged to the byte and a clean case pays nothing for them: ~28.2 MB and 46.6 ms against 37.7 MB and 54.4 ms on a 2869-bus/4000-branch case.
